#!/usr/bin/env node
// Run: node build.js
// Reads data.js and writes a fully static index.html.
// All content is pre-baked — no JavaScript needed to read it.
...
function renderToolIcon(icon) {
  if (!icon) return "";
  if (isImgPath(icon)) {
    return '<span class="tool-icon"><img src="' + esc(icon) + '" alt=""></span>';
  }
  return '<span class="tool-icon">' + esc(icon) + "</span>";
}

function renderMediaItem(item) {
  var inner = "";
  if (item.type === "image") {
    inner = '<img src="' + esc(item.src) + '" alt="' + esc(item.caption || "") + '">';
  } else if (item.type === "html") {
    inner = '<iframe src="' + esc(item.src) + '" title="' + esc(item.caption || "Preview") +
      '" frameborder="0" loading="lazy" sandbox="allow-scripts allow-same-origin"></iframe>';
...
function renderProject(proj, idx) {
  var isOrange  = idx % 2 === 0;
  var accent    = isOrange ? "orange" : "yellow";
  var boxClass  = isOrange ? "box-orange" : "box-yellow";
  var media     = proj.media || [];
  var hasNav    = media.length > 1;

  var mediaItemsHTML = "";
  media.forEach(function(item, mi) {
    mediaItemsHTML +=
      '<div class="media-item" data-mi="' + mi + '" style="display:' + (mi === 0 ? "block" : "none") + '">' +
        renderMediaItem(item) +
      "</div>";
  });

  var noMedia = media.length === 0
    ? '<div style="flex:1;display:flex;align-items:center;justify-content:center;font-size:0.84rem;padding:16px">No preview</div>'
    : "";
  ...
  if (hasNav) {
    navHTML =
      '<div class="media-nav">' +
        '<button class="media-nav-btn" data-proj="' + idx + '" data-dir="-1" disabled>&#8249;</button>' +
        '<span class="media-counter"><span class="media-cur">1</span>&nbsp;/&nbsp;' + media.length + "</span>" +
        '<button class="media-nav-btn" data-proj="' + idx + '" data-dir="1">&#8250;</button>' +
      "</div>";
  }
  ...
  return (
    '<div class="project-card ' + boxClass + '" data-accent="' + accent + '" data-proj="' + idx + '">' +
      '<div class="project-media">' +
        '<div class="media-viewer">' + noMedia + mediaItemsHTML + "</div>" +
        navHTML +
      "</div>" +
...
function buildContent(data) {
  ...
  // ── Introduction ────────────────────────────────────────
  var roles = data.intro.roles || [];

  var roleBtnsHTML = roles.map(function(r, i) {
    return '<li><button class="role-btn' + (i === 0 ? " active" : "") + '" data-ri="' + i + '">' + esc(r.title) + "</button></li>";
  }).join("");

  var roleSummariesHTML = roles.map(function(r, i) {
    return '<div class="role-summary-item' + (i === 0 ? " active" : "") + '" data-ri="' + i + '">' + esc(r.summary) + "</div>";
  }).join("");
  ...
  html +=
    ...
    '<div class="intro-grid">' +
      '<div class="intro-bio box-orange">' +
        ...
        '<div class="intro-bio-text"><p>' + esc(data.intro.bio) + "</p></div>" +
      "</div>" +
      '<div class="intro-roles box-green">' +
        '<div class="intro-roles-label">Professional Roles</div>' +
        '<div class="roles-layout">' +
          '<ul class="roles-list">' + roleBtnsHTML + "</ul>" +
          '<div class="role-summaries">' + roleSummariesHTML + "</div>" +
        "</div>" +
      "</div>" +
    "</div>";

  // ── Certifications ────────────────────────────────────
  var certCards = (data.certifications || []).map(function(cert) {
    var logoHTML = cert.logo
      ? '<div class="cert-logo-wrap"><img class="cert-logo" src="' + esc(cert.logo) + '" alt="' + esc(cert.name) + ' logo"></div>'
      : '<div class="cert-logo-wrap"><div class="cert-logo-placeholder" aria-hidden="true">&#127885;</div></div>';
    return (
      '<div class="cert-card box-orange">' +
        logoHTML +
        '<div class="cert-name">' + esc(cert.name) + "</div>" +
        '<div class="cert-desc">' + esc(cert.description) + "</div>" +
      "</div>"
    );
  }).join("");

  // ── Projects ──────────────────────────────────────────
  var projectsHTML = (data.projects || []).map(function(proj, i) {
    return renderProject(proj, i);
  }).join("");
  ...
  var processCards = (data.processImprovements || []).map(function(pi) {
    var bullets = (pi.bullets || []).map(function(b) {
      return "<li>" + esc(b) + "</li>";
    }).join("");
    return (
      '<div class="process-card box-green">' +
        '<p class="process-summary">' + esc(pi.summary) + "</p>" +
        '<ul class="process-bullets">' + bullets + "</ul>" +
      "</div>"
    );
  }).join("");

  html +=
    '<h2 class="section-heading">Process Improvements</h2>' +
    '<div class="carousel-wrapper"><div class="carousel-track">' + processCards + "</div></div>";

  html +=
    '<h2 class="section-heading">Certifications</h2>' +
    '<div class="carousel-wrapper"><div class="carousel-track">' + certCards + "</div></div>";

  // ── Skills ────────────────────────────────────────────
  function renderSkillRow(item) {
    return (
      '<li class="skill-row">' +
        '<span class="skill-icon">' + renderToolIcon(item.icon) + "</span>" +
        '<span class="skill-name">' + esc(item.title) + "</span>" +
      "</li>"
    );
  }

  var toolRows  = (data.skills.tools     || []).map(renderSkillRow).join("");
  var techRows  = (data.skills.technical || []).map(renderSkillRow).join("");
  var softRows  = (data.skills.soft      || []).map(renderSkillRow).join("");

  html +=
    '<h2 class="section-heading">Skills</h2>' +
    '<div class="skills-grid">' +
      '<div class="box-yellow skills-box">' +
        '<div class="skills-col-label">Tools</div>' +
        '<ul class="skill-rows">' + toolRows + "</ul>" +
      "</div>" +
      '<div class="box-yellow skills-box">' +
        '<div class="skills-col-label">Technical Skills</div>' +
        '<ul class="skill-rows">' + techRows + "</ul>" +
      "</div>" +
      '<div class="box-yellow skills-box">' +
        '<div class="skills-col-label">Soft Skills</div>' +
        '<ul class="skill-rows">' + softRows + "</ul>" +
      "</div>" +
    "</div>";

  // ── Other Projects ────────────────────────────────────
  var otherItems = (data.otherProjects || []).map(function(op) {
    var href = op.url && op.url !== "#"
      ? ' href="' + esc(op.url) + '" target="_blank" rel="noopener"'
      : ' href="#"';
    return (
      '<div class="other-project-item box-green">' +
        '<a class="other-project-link"' + href + ">" + esc(op.title) + "</a>: " +
        esc(op.description) +
      "</div>"
    );
  }).join("");
  ...
  html +=
    ...
    '<div class="other-projects-list">' + otherItems + "</div>";
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Domine:wght@400;700&family=Mulish:wght@400;600;700;800&display=swap" rel="stylesheet">
  <style>
    /* ── RESET ─────────────────────────────────────────── */
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    :root {
      ...
      --box-orange-bg:     #fff0e6;
      --box-orange-border: #f25800;
      --box-yellow-bg:     #fdf8d4;
      --box-yellow-border: #c9ac00;
      --box-green-bg:      #edf5ee;
      --box-green-border:  #004209;
    ...
    @media (prefers-color-scheme: dark) {
      :root {
        ...
        --box-orange-bg:     #321500;
        --box-orange-border: #c44800;
        --box-yellow-bg:     #251d00;
        --box-yellow-border: #b89600;
        --box-green-bg:      #001a05;
        --box-green-border:  #1a6b28;
    ...
    @media (prefers-color-scheme: dark) {
      html { background-image: url("data:image/svg+xml;base64,${fleurDark}"); }
    }
    ...
    /* ── BOX — single shared style ──────────────────────── */
    .box-orange {
      background: var(--box-orange-bg);
      border: 2px solid var(--box-orange-border);
    }

    .box-yellow {
      background: var(--box-yellow-bg);
      border: 2px solid var(--box-yellow-border);
    }

    .box-green {
      background: var(--box-green-bg);
      border: 2px solid var(--box-green-border);
    }
    ...
    .media-nav-btn:hover:not(:disabled) { background: var(--border); }
    .media-nav-btn:disabled { opacity: 0.3; cursor: default; }

    .media-counter { font-size: 0.76rem; }
    ...
    .other-project-link {
      font-weight: 700;
      color: var(--box-green-border);
      text-decoration: underline;
    }

    .other-project-link:hover { opacity: 0.8; }
    ...
    @media (max-width: 768px) {
      ...
      .section-heading { font-size: 1.55rem; margin-top: 48px; }
...
${content}
  ...
  <!-- Progressive enhancement: role tabs and media carousel.
       All content above is readable without this script. -->
  <script>
    (function () {
      "use strict";
      ...
      var roleSummaryItems = document.querySelectorAll(".role-summary-item");
      roleBtns.forEach(function (btn) {
        btn.addEventListener("click", function () {
          var ri = parseInt(btn.getAttribute("data-ri"), 10);
          roleBtns.forEach(function (b) { b.classList.remove("active"); });
          btn.classList.add("active");
          roleSummaryItems.forEach(function (item) {
            item.classList.toggle("active", parseInt(item.getAttribute("data-ri"), 10) === ri);
          });
        });
      });
      ...
      document.querySelectorAll(".media-nav-btn").forEach(function (btn) {
        btn.addEventListener("click", function () {
          var projIdx = parseInt(btn.getAttribute("data-proj"), 10);
          var dir     = parseInt(btn.getAttribute("data-dir"), 10);
          var card    = document.querySelector('.project-card[data-proj="' + projIdx + '"]');
          if (!card) return;
          var items = card.querySelectorAll(".media-item");
          if (mediaState[projIdx] == null) mediaState[projIdx] = 0;
          var next = Math.max(0, Math.min(items.length - 1, mediaState[projIdx] + dir));
          mediaState[projIdx] = next;
          items.forEach(function (item) {
            item.style.display = parseInt(item.getAttribute("data-mi"), 10) === next ? "block" : "none";
          });
          var curEl = card.querySelector(".media-cur");
          if (curEl) curEl.textContent = next + 1;
          card.querySelectorAll(".media-nav-btn").forEach(function (b) {
            var d = parseInt(b.getAttribute("data-dir"), 10);
            b.disabled = (d === -1 && next === 0) || (d === 1 && next === items.length - 1);
          });
        });
      });
    })();
